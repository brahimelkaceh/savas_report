...
export default function EditLot({ lot, setRefetchData }) {
  ...
  const fetchBatsData = async (id) => {
    try {
      if (!id) {
        return;
      }
      setLoading(true);
      const result = await api.getBatsData(id);
      const filteredData = result?.filter(
        (data) => data.type === "poussiniere"
      );
